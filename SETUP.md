...
For setup on MAC OSX using BSD sed use the following commands.

```sh
# Edit ci/deploy.sh with travis secure env variables generated by travis encrypt-file
TRAVIS_ENCRYPT_ID=`sed \
    -n 's/.*encrypted_\(.*\)_key.*/\1/p' \
    travis-encrypt-file.log`
sed -i '' "s/f2f00aaf6402/$TRAVIS_ENCRYPT_ID/g" deploy.sh
```
...
```sh
sed -i '' "s/greenelab/$OWNER/g" README.md
sed -i '' "s/manubot-rootstock/$REPO/g" README.md
