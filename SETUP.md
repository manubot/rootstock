...
TRAVIS_ENCRYPT_ID=`python -c "import re; \
    print(re.search('encrypted_([a-zA-Z0-9]+)_key', \
    open('travis-encrypt-file.log').read()).group(1))"`
sed "s/f2f00aaf6402/$TRAVIS_ENCRYPT_ID/g" deploy.sh > tmp
mv -f tmp deploy.sh
```
...
sed "s/greenelab/$OWNER/g" README.md > tmp
mv -f tmp README.md
sed "s/manubot-rootstock/$REPO/g" README.md > tmp
mv -f tmp README.md
