...
For example, to manually specify the metadata for the citation `@url:https://github.com/manubot/rootstock`, add a CSL JSON Item to `manual-references.json` that contains:
...
For example, to cite `@raw:private-message` in a manuscript, a corresponding CSL Item is required, such as:
...
So, for example, to provide metadata for `@url:https://github.com/manubot/rootstock` in BibTeX format, create the file `content/manual-references.bib` and create an item whose definition starts with:
...
In order to freeze all references, rather than have Manubot regenerate them during future builds, copy the output `references.json` file to `content` as a manual references file.
